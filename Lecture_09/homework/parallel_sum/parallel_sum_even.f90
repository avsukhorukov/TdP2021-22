...
!
! Compile and run:
! $ mpifort -g -O0 -Wall -Wextra -Wpedantic -fcheck=all -fbacktrace parallel_sum_even.f90
! $ mpirun -np 4 --oversubscribe ./a.out < in16.txt
! 136
!
! TODO: students get confused about sending the local size of a subarray.  It is
! better to change the problem conditions and ask to send the full size and let
! all ranks to calculate necessary sizes locally.
program parallel_sum_even
    ...
    ! MPI_COMM_WORLD name to a communicator variable to avoid typing this long
    ! name.
    comm = MPI_COMM_WORLD
